...
using DMTimeArea;
using System;
using UnityEditor.Skill;

public class SkillEditor : SimpleTimeArea
{
	private SkillView skillView = new SkillView();
	private List<SkillTrackItem> skillTrackLst = new List<SkillTrackItem>();
	private Vector2 scrollPos = Vector2.zero;
	private SkillTrackItem currTrack = null;
	...
	protected override double RunningTime
	{
		...
		{
			runningTime = value;
			skillTrackLst.ForEach(c => c.OnRunTimeChanged(runningTime, IsPlaying));
		}
	}
	...
	[MenuItem("Tools/SkillEditor", false, 2002)]
	...
	private void OnEnable()
	{
		...
		var clip = new ClipTrackItem();
		clip.Set(this);
		clip.Set("xxx");
		clip.OnSelect = OnSelectHandler;
		skillTrackLst.Add(clip);

		for (int i = 0; i < 5; i++)
		{
			var item = new SkillTrackItem();
			item.Set(this);
			item.OnSelect = OnSelectHandler;
			skillTrackLst.Add(item);
		}
	}
	...
	private void OnGUI()
	{
		Rect rectMainBodyArea = new Rect(5, 0, base.position.width, this.position.height);
	...
	protected virtual void DrawContent()
	{
		...
		var z = 0f;
		for (int i = 0; i < skillTrackLst.Count; i++)
		{
			skillTrackLst[i].DrawInfo(ref z, rectLeft.width);
			z += 5;
		}
		...
		var rightView = rectContent;
		rightView.width -= 15;
		GUILayout.BeginArea(rightView);
		GUILayout.BeginVertical();
		scrollPos = GUILayout.BeginScrollView(scrollPos);
		z = 22f;
		for (int i = 0; i < skillTrackLst.Count; i++)
		{
			skillTrackLst[i].DrawTrack(ref z, rectTimeRuler);
			z += 5;
		}
	...
	{
		// left top tool bar
		GUILayout.BeginArea(rectLeftTopToolBar, string.Empty, EditorStyles.toolbar);
	...
	private void OnSelectHandler(SkillTrackItem item)
	{
		currTrack?.SetSelect(false);
		currTrack = item;
		currTrack.SetSelect(true);
		GUI.changed = true;
	}
